...
void displayNumber(s16 number)
{
	...
	// If the finalResult is negative, represented by absoluteValue and a sign '-'
	else if (number < 0 && number >= -999) {
		absoluteValue = -number;
		calculateDigits(absoluteValue);
		//the capacity is 4, and considering the Dash will take one position
		//the position index: 0 1 2 3;
		//give the dash to represent minus sign for negative finalResult;
